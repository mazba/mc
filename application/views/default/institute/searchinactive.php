...
<div id="system_content">
    ...
    <div id="system_content" class="dashboard-wrapper">
        <div class="grid_10" >
            ...
            <form action="<?php echo $CI->get_encoded_url('institute/institute/index/searchinactive'); ?>" class="signup" method="post" accept-charset="utf-8" enctype="multipart/form-data" id="frm_user_approval">
                <div class="row widget">
                    <div class="widget-header">
                        <div class="title">
                            <?php echo $this->lang->line('INSTITUTE_REGISTRED_SEARCH_INACTIVE');?>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-xs-4">
                            <label class="control-label pull-right"><?php echo $CI->lang->line('INSTITUTE_SEARCH'); ?></label>
                        </div>
                        <div class="col-sm-4 col-xs-8">
                            <input type="text" name="email" class="selectbox-1 form-control" value="" />
                        </div>
                    </div>
                    ...
                    <div class="row show-grid">
                        ...
                        <div class="col-sm-4 col-xs-8">
                            <input type="submit" id="search" name="1" class="btn btn-primary" value="<?php echo $CI->lang->line('SEARCH'); ?>">
                            <input type="button" style="cursor:pointer;" id="reset" class="btn btn-primary" name="1" value="<?php echo $this->lang->line('RESET');?>" />
                        </div>
                    </div>
        ...
        function disable(id)
        {
            ...
            if(r==true){
             //   alert(article_id);
                $.ajax({
                    type:"POST",
                    url: '<?php echo $CI->get_encoded_url('institute/institute/disable/'); ?>',
                    data: 'id=' + id,
...
//            else{
//                window.location.href = '<?php //echo $CI->get_encoded_url('institute/institute/index/searchinactive'); ?>//';
//            }
        }

        function approved(id)
        {
            var r=confirm("Can you confirm approved this?")
            if(r==true){
                //   alert(article_id);
                $.ajax({
                    type:"POST",
                    url: '<?php echo $CI->get_encoded_url('institute/institute/quicksave/'); ?>',
                    data: 'id=' + id,
                    success: function(result){
                        //    alert("sdsd");
                        if(result.delete_status=="DELETE")
        ...
        <table class="table table-bordered table-hover table-striped" style="width: 100%">
            ...
            <tr  id="tr_<?php echo$result['id']; ?>">
                ...
                <td>
                    ...
                    <button class="btn btn-primary btn-block" onclick="approved('<?php echo$result['id']; ?>')"><?php echo $CI->lang->line('APPROVED'); ?></button>
                    <button class="btn btn-primary btn-block" onclick="disable('<?php echo$result['id']; ?>')"><?php echo $CI->lang->line('DISCARD'); ?></button>
