...
class System_helper
{
    ...
    // This function return the name of user division/district/upozila name
    //Create By Antu Rozario
    public static function get_user_division_district_upozila_name(){
        $CI = &get_instance();
        ...
        if ($user->user_group_id == $CI->config->item('SUPER_ADMIN_GROUP_ID')) {
            ...
            $CI =& get_instance();
            $CI->db->select('users.division');
            $CI->db->select('division.divname');
            $CI->db->from($CI->config->item('table_users').' users');
            $CI->db->join($CI->config->item('table_divisions').' division','division.divid = users.division', 'LEFT');
            $CI->db->where('users.id =',$user->id);
            $query=$CI->db->get()->row();
            //echo $this->db->last_query();
            return $query->divname;
            ...
            $CI =& get_instance();
            $CI->db->select('users.zilla');
            $CI->db->select('zilla.zillaname');
            $CI->db->from($CI->config->item('table_users').' users');
            $CI->db->join($CI->config->item('table_zillas').' zilla','zilla.zillaid = users.zilla', 'LEFT');
            $CI->db->where('users.id =',$user->id);
            $query=$CI->db->get()->row();
            //echo $this->db->last_query();
            return $query->zillaname;
            ...
            $CI->db->from($CI->config->item('table_upazilas').' upazila');
            $CI->db->where('upazila.zillaid ', SELF::int_value($user->zilla));
            $CI->db->where('upazila.upazilaid ', SELF::int_value($user->upazila));

            $query=$CI->db->get()->row();

            return $query->upazilaname;
        } else {
            //$CI->db->where('','');table_upazilas
        }

}
    public static function int_value($str)
    {
        if (strlen($str) == 1) {
            $number = "0" . $str;
        } else {
