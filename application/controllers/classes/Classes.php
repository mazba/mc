...
class Classes extends Root_Controller
{
    public $permissions;
    public $message;
    ...
    function __construct()
    {
        parent::__construct();
        $this->message='';
        $this->permissions=Menu_helper::get_permission('classes');
        $this->controller_url='classes/classes';
    ...
    private function system_list()
    {
        if($this->permissions['list'])
        {
            ...
            $ajax['system_content'][]=array("id"=>"#system_wrapper","html"=>$this->load_view("classes/system_list","",true));

            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
            $ajax['system_page_url']=$this->get_encoded_url('classes/classes');
            $ajax['system_page_title']=$this->lang->line("CLASS");
    ...
    private function system_add()
    {
        if($this->permissions['add'])
        {
            ...
            $ajax['status']=true;
            $data=array();
            $data['title']=$this->lang->line("CREATE_NEW_CLASSES");
            $data['record_info']['id']=0;
            $data['record_info']['name']='';
            $data['record_info']['education_type']='';
            $data['record_info']['education_level_id']='';
            ...
            $ajax['system_content'][]=array("id"=>"#top_header","html"=>$this->load_view("header",'',true));
            $ajax['system_content'][]=array("id"=>"#system_wrapper_top_menu","html"=>$this->load_view("top_menu","",true));
            $ajax['system_content'][]=array("id"=>"#system_wrapper","html"=>$this->load_view("classes/system_add_edit",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
            $ajax['system_page_url']=$this->get_encoded_url('classes/classes/index/add');
            $this->jsonReturn($ajax);
        }
        else
        {
    ...
    private function system_edit($id)
    {
        ...
        if($this->permissions['edit'])
        {
            ...
            $ajax['status']=true;
            $data=array();
            $data['title']=$this->lang->line("EDIT_CLASSES");
            $data['record_info']=Query_helper::get_info($this->config->item('table_classes'),'*',array('id ='.$id),1);
            $data['education_type'] = Query_helper::get_list($this->config->item('table_education_type'),'name',array('status = 1'));
            $data['education_level'] = Query_helper::get_list($this->config->item('table_education_level'),'name',array('status = 1'));
            ...
            $ajax['system_page_url']=$this->get_encoded_url('classes/classes/index/edit/'.$id);
            $this->jsonReturn($ajax);
        }
        else
        {
    ...
    private function system_save()
    {
        ...
        else
        {
            ...
            if($this->classes_model->check_duplicate($id,$data)){
                $ajax['status']=false;
            ...
            if($id>0)
            {
                ...
                Query_helper::update($this->config->item('table_classes'),$data,array("id = ".$id));
            ...
            else
            {
                ...
                $this->db->trans_start();  //DB Transaction Handle START
                Query_helper::add($this->config->item('table_classes'),$data);
                $this->db->trans_complete();   //DB Transaction Handle END
    ...
    private function system_batch_details()
    {
        if($this->permissions['view'])
        {
            ...
            $selected_ids=$this->input->post('selected_ids');
            $data['user_groups']=$this->classes_model->get_user_group_details($selected_ids);
            $ajax['status']=true;

            $ajax['system_content'][]=array("id"=>"#system_wrapper_top_menu","html"=>$this->load_view("top_menu","",true));
            $ajax['system_content'][]=array("id"=>"#system_wrapper","html"=>$this->load_view("classes/system_details",$data,true));
            $this->jsonReturn($ajax);
        }
        else
        {
    ...
    private function system_batch_delete()
    {
        if($this->permissions['delete'])
        {
            ...
            {
                Query_helper::update($this->config->item('table_classes'),array('status'=>99,'update_by'=>$user->id,'update_date'=>time()),array("id = ".$id));
            }
            $this->db->trans_complete();   //DB Transaction Handle END
    ...
    private function check_validation()
    {
        ...
        $this->form_validation->set_rules('name',$this->lang->line('NAME'),'required',array('required' => $this->lang->line('CLASS_NAME')));
    ...
    public function get_all()
    {
        ...
        if($this->permissions['list'])
        {
            $groups= $this->classes_model->get_all();
